...
<h3>${param.year } 년 월별 매출</h3>
	<div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
	<form>
		<select name="year" id="year">
			<option value=0>--</option>
		  	<c:forEach items="${years }" var="vo">
		  		<option value="${vo.year }" <c:out value="${param.year eq vo.year? 'selected' : '' }"/>><c:out value="${vo.year }"></c:out></option>
		  	</c:forEach>
	  	</select>
    ...
    $(document).ready(function(){
      ...
      $("#searchYear").on("click", function(){
        ...
        if(year == 0){
          ...
          location.href = "/admin/chart/searchStatistics?year="+year;
        }
      });
    });
  </script>
